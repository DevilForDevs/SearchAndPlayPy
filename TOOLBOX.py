...
def txt2filename(txt):
    special_characters = ['@', '#', '$', '*', '&', '<', '>', '/', '\b', '|', '?', 'CON', ' PRN', ' AUX', ' NUL',
                          ' COM0',
...
def shel_rendtrer(item):
    ...
    if "verticalListRenderer" in subitem:
        ivd = subitem["verticalListRenderer"]["items"]  # subitem["verticalListRenderer"]["items"]
        for ivids in ivd:
            vide_id = ivids["videoRenderer"]["videoId"]
            title = ivids["videoRenderer"]["title"]["runs"][0]["text"]  # thumbnail,title,lengthText
    ...
    if "horizontalListRenderer" in subitem:
        ...
        for v in hrzitems:
            if "gridVideoRenderer" in v:
                vid = v["gridVideoRenderer"]["videoId"]  # videoId', 'thumbnail', 'title'
    ...
    return itims
    # if "horizontalCardListRenderer" in subitem:
    #     print("subiten", subitem["horizontalCardListRenderer"])
...
def search(term, conti):
    ...
    # dict_keys(['responseContext', 'estimatedResults', 'contents', 'trackingParams', 'header', 'topbar', 'targetId'])
...
def scrap_data(vid):
    ...
    query = {"videoId": vid, 'playbackContext': {'contentPlaybackContext': {'html5Preference': 'HTML5_PREF_WANTS'}},
             'contentCheckOk': True, 'racyCheckOk': True}
    data = {'context': {'client': {'clientName': 'IOS', 'clientVersion': '17.33.2', "deviceModel": "iPhone14,3",
...
# https://www.youtube.com/watch?v=HH1PXnKg2LU&pp=ygUNa2FiaXIga2UgZG9oZQ%3D%3D
def video_id(url: str) -> str:
    if regex_search(r"(?:v=|\/)([0-9A-Za-z_-]{11}).*", url, group=1):
